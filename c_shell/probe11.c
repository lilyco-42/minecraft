...
static FILE *f;
static void logline(const char *s) { f = fopen("probe11.log","a"); if(f){fprintf(f,"%s\n",s);fclose(f);} }
...
int APIENTRY wWinMain(HINSTANCE hInst, HINSTANCE prev, PWSTR cmd, int show) {
    (void)prev; (void)cmd;
    DeleteFileW(L"probe11.log");
    logline("start");
    WNDCLASSW wc = {0};
    wc.lpfnWndProc = WP;
    wc.hInstance = hInst;
    wc.lpszClassName = L"P11";
    RegisterClassW(&wc);
    g_hwnd = CreateWindowExW(0, L"P11", L"P11", WS_OVERLAPPEDWINDOW, 100,100,1000,700, NULL,NULL,hInst,NULL);
